...
async def check_existence_with_name(database, name:str):
    obj = await database.find({"name":name})
    if not obj:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Object with supplied name doesn't exist"
        )
    return obj
...
async def check_authority(name: str, array):
